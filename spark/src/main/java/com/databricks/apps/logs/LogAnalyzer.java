...
public class LogAnalyzer {
	...
	public static void main(String[] args) {
		...
		//Log Analyzer
	    SparkConf conf = new SparkConf().setMaster("local[1]").setAppName("Log Analyzer");
   ...
   // JavaRDD<String> logLines = sc.textFile("C:\\Users\\mike Obijaju\\Desktop\\access_log2.txt");
	   JavaRDD<String> logLines = sc.textFile(logFile);
	    ...
	    //   accessLogs.map(ApacheAccessLog::getByteSize).cache();
...
//	        accessLogs.mapToPair(log -> new Tuple2<>(log.getResponse(), 1L))
...
//	    System.out.println(String.format("Response code counts: %s", responseCodeToCount));
	    ...
	    List<Tuple2<String, Long>> topEndpoints = accessLogs
	        .mapToPair(log -> new Tuple2<>(log.getRequest(), 1L))
	        .reduceByKey(SUM_REDUCER)
